<script lang="ts">
  ...
  function handleNewNode(node:INode){
    ...
    let deepCopy = JSON.parse(JSON.stringify({categories,title,description,inputs,outputs}))
...
<ul class="menu bg-base-100 w-56 p-2 rounded-box absolute" style="top:{$mouseClicks.down?.clientY}px;left:{$mouseClicks.down?.clientX}px;" class:hidden={!$contextVisible} id="ctxmenu">
    {#each categories as category}
        ...
        {#each nodes_by_category[category] as node}
            ...
            <li on:click={()=>{handleNewNode(node)}}><a>{node.title}</a></li>
        {/each}
    {/each}
</ul>