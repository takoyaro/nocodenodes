...
export const mouseClicks: Writable<{
  down: MouseEvent | null;
  up: MouseEvent | null;
}> = writable({ down: null, up: null });
...
export const LinkBuilder: Writable<{
  from: { node: NodeComponent | undefined | null; pinIndex: number | null };
  to: { node: NodeComponent | undefined | null; pinIndex: number | null };
}> = writable({
  from: {
    node: null,
    pinIndex: null,
