<script lang="ts">
	import { contextVisible, iH, isPanning, iW, keyEvents, mouse, mouseClicks, viewBox, Zoom } from "./store";
	...
	function handleMove(e:MouseEvent){
		...
		if($isPanning && e.buttons){
			...
			viewBox.set([$viewBox[0]+=viewboxX_delta,$viewBox[1]+=e.movementY*-1*$Zoom]);
		}
	}
...
on:mouseup={(e)=>{$mouseClicks.down=null}}
