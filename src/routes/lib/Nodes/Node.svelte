<script lang="ts">
	...
	import { iW, mouse, mouseClicks, nodes, viewBox, Zoom } from "../../store";
  import { ECategories } from "./ENUMS";
    ...
    $:outputCount = Array.isArray(NodeDef.outputs) ? NodeDef.outputs.length ?? 0 : 1;
    ...
    export let [nodeX,nodeY] = [parseInt(`${$viewBox[0]+($mouse.clientX)-nodeWidth/2}`),parseInt(`${$viewBox[1]+($mouse.clientY)}`)]
    mouseClicks.subscribe((e)=>{
		if(e){
			if(e.down?.buttons){ //mouseDown
    ...
    export function updateConnections(){
        ...
        for (let index = 0; index < NodeDef.inputs.length; index++) {
            ...
            if(conn.from){
                const matchingOutputs = [...$nodes].filter(([s,n],i)=>{return n.NodeDef.outputs.some(p=>p.to==NodeDef.UUID)})
                console.log("Propagating from ",conn.from, " to some of ",matchingOutputs)
                ...
                conn.value = node?.NodeDef.outputs.filter(o=>o.to == uuid).at(0)?.value
            }
        }
        for (let index = 0; index < NodeDef.outputs.length; index++) {
            ...
            if(conn){
                if(conn.to){
                    const matchingInputs = [...$nodes].filter(([s,n],i)=>{return n.NodeDef.inputs.some(p=>p.from==NodeDef.UUID)});
...
<g style="translate:{nodeX}px {nodeY}px;" id={uuid}>
    ...
    {/each} -->
    <foreignObject width="{nodeWidth}" height="{height}" x=0 y=0>
        <div class="node">
            ...
            {#if inputCount>0}
                {#each NodeDef.inputs as input,i}
                    ...
                    {@const TotalPositionFromTop = posFromTopToInputTop+InputIndexHeight+pinIndexAdjustment}
                    ...
                    <div class="flex flex-row">
                        ...
                        on:click={(e)=>{handleLinkTo(i)}}></div>
                        <input placeholder="{input.label}" bind:value={input.value} on:keyup={updateConnections} disabled={hasConnectionIn} class:input-disabled={hasConnectionIn}/>
                    </div>
                {/each}
            {:else if NodeDef.categories.includes(ECategories.CONSTS)}
                <input placeholder="{NodeDef.outputs[0].label}" style="width:{nodeWidth/2}px;" bind:value={NodeDef.outputs[0].value} on:keyup={updateConnections}/>
            {/if}
            {#each NodeDef.outputs as output,i}
                ...
                {@const nodeMargin = padding*2}
                {@const posFromTopToInputTop = padding+nodePadding+nodeMargin}
                {@const InputIndexHeight = inputTotalPadding*indexAdjustment}
                ...
                on:click={(e)=>{handleLinkFrom(i)}}></div>
